...
It is all these small variations in the sound wave, specific to the instrument, that give it its unique character. Physically speaking, they correspond to *frequencies* of the sound wave, and it is the way an instrument modulates all the frequencies (the "spectrum") that textures the sound. Among all those frequencies, one is more represented than all the others; if we keep only this one, we will get a perfect sine wave:
